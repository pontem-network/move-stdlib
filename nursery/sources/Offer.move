...
module Std::Offer {
    ...
    const ERR_OFFER_DNE_FOR_ACCOUNT: u64 = 0;

    /// Address already has an offer of this type.
    const ERR_OFFER_ALREADY_CREATED: u64 = 1;

    /// Address does not have an offer of this type to redeem.
    const ERR_OFFER_DOES_NOT_EXIST: u64 = 2;
    ...
    public fun create<Offered: store>(account: &signer, offered: Offered, for: address) {
        assert!(!exists<Offer<Offered>>(Signer::address_of(account)), Errors::already_published(ERR_OFFER_ALREADY_CREATED));
    ...
    public fun redeem<Offered: store>(account: &signer, offer_address: address): Offered acquires Offer {
        assert!(exists<Offer<Offered>>(offer_address), Errors::not_published(ERR_OFFER_DOES_NOT_EXIST));
        let Offer<Offered>{ offered, for } = move_from<Offer<Offered>>(offer_address);
        let sender = Signer::address_of(account);
        assert!(sender == for || sender == offer_address, Errors::invalid_argument(ERR_OFFER_DNE_FOR_ACCOUNT));
        offered
    }
    spec redeem {
