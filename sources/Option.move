...
module Std::Option {
    ...
    const ERR_OPTION_IS_SOME: u64 = 0;
    /// The `Option` is in an invalid state for the operation attempted.
    /// The `Option` is `None` while it should be `Some`.
    const ERR_OPTION_IS_NONE: u64 = 1;
    ...
    public fun borrow<Element>(t: &Option<Element>): &Element {
        assert!(is_some(t), Errors::invalid_argument(ERR_OPTION_IS_NONE));
    ...
    public fun fill<Element>(t: &mut Option<Element>, e: Element) {
        ...
        else abort Errors::invalid_argument(ERR_OPTION_IS_SOME)
    }
    spec fill {
        pragma opaque;
    ...
    public fun destroy_none<Element>(t: Option<Element>) {
        assert!(is_none(&t), Errors::invalid_argument(ERR_OPTION_IS_SOME));
        let Option { vec } = t;
        Vector::destroy_empty(vec)
    }
    spec destroy_none {
