...
module Std::FixedPoint32 {
    ...
    public fun multiply_u64(val: u64, multiplier: FixedPoint32): u64 {
        ...
        assert!(product <= MAX_U64, Errors::limit_exceeded(ERR_MULTIPLICATION));
        (product as u64)
    }
    ...
    public fun divide_u64(val: u64, divisor: FixedPoint32): u64 {
        ...
        assert!(divisor.value != 0, Errors::invalid_argument(ERR_DIVISION_BY_ZERO));
        // First convert to 128 bits and then shift left to
        // add 32 fractional zero bits to the dividend.
        let scaled_value = (val as u128) << 32;
        let quotient = scaled_value / (divisor.value as u128);
        // Check whether the value is too large.
        assert!(quotient <= MAX_U64, Errors::limit_exceeded(ERR_DIVISION));
    ...
    public fun create_from_rational(numerator: u64, denominator: u64): FixedPoint32 {
        ...
        assert!(scaled_denominator != 0, Errors::invalid_argument(ERR_ZERO_DENOMINATOR));
        let quotient = scaled_numerator / scaled_denominator;
        assert!(quotient != 0 || numerator == 0, Errors::invalid_argument(ERR_RATIO_OUT_OF_RANGE));
        // Return the quotient as a fixed-point number. We first need to check whether the cast
        // can succeed.
        assert!(quotient <= MAX_U64, Errors::limit_exceeded(ERR_RATIO_OUT_OF_RANGE));
